...
- New semantic token `semantic.color.borderHovered` (light: `color.neutral.700`, dark: `color.darkNeutral.700`) sits between `borderBold` and the focused state, giving hover its own intensity step on top of the existing `border` / `borderBold` / `borderDisabled` / `borderFocused` scale.
